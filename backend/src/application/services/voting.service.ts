import type { IIdeasRepository, IVotesRepository } from '../../domain/interfaces/repositories.js';
...
export class VotingService {
  ...
  async vote(ideaId: number, ipAddress: string): Promise<VoteResult> {
    ...
    const alreadyVoted = await this.votesRepository.existsByIdeaAndIp(ideaId, ipAddress);
    if (alreadyVoted) {
      throw new VotingError(
